...
func TestChooseNATAnchorUsesObservedSubnet(t *testing.T) {
	...
	if choice.IP == "192.168.1.42" || choice.IP == "192.168.1.1" {
	...
	if ip := net.ParseIP(choice.IP); ip == nil || !(&net.IPNet{IP: net.ParseIP("192.168.1.0"), Mask: id.Netmask}).Contains(ip) {
...
func TestChooseNATAnchorFallsBackWithoutObservedMask(t *testing.T) {
	...
	if got, want := choice.IP, "10.254.254.10"; got != want {
